  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        {[
          { label: "Pending Review", value: pending.length, color: "from-yellow-500 to-orange-500" },
          { label: "Approved", value: approved.length, color: "from-green-500 to-emerald-500" },
          { label: "Total Applications", value: applications.length, color: "from-cyan-500 to-teal-500" },
        ].map((stat, index) => (
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl p-10 text-center shadow"><p className="text-gray-500">No applications found.</p></div>
      ) : (
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setSelectedApplication(null)}>
          <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white rounded-xl shadow-2xl p-8 max-w-3xl w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
                  ...
                  {["idDocument", "licenseDocument", "insuranceDocument"].map((doc) => (
                    <div key={doc} className="bg-white rounded-lg p-4 text-center">
                      ...
                      <p className="text-xs font-medium text-gray-600">
                        {doc === "idDocument" ? "ID" : doc === "licenseDocument" ? "License" : "Insurance"}
                      </p>
                      <p className="text-xs text-gray-400">{selectedApplication.documents?.[doc] ? "Uploaded" : "Missing"}</p>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="flex gap-4 mt-8">
              <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => setSelectedApplication(null)} className="flex-1 px-6 py-3 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-colors">Close</motion.button>
              <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => handleStatus(selectedApplication._id, "rejected")} className="flex-1 px-6 py-3 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors">Reject</motion.button>
              <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => handleStatus(selectedApplication._id, "approved")} className="flex-1 px-6 py-3 bg-gradient-to-r from-cyan-600 to-teal-600 text-white rounded-lg shadow-lg hover:shadow-xl transition-shadow">Approve</motion.button>
            </div>
          </motion.div>
        </motion.div>
      )}
