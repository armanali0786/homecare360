...
export function ReviewsModeration() {
  ...
  const handleStatus = async (id: string, status: string) => {
    try {
      ...
      toast.success(t("reviews.toast.statusUpdated", { status: t(`reviews.statusLabel.${status}`, { defaultValue: status }) }));
      fetchReviews();
    } catch (err: any) {
      toast.error(err.message || t("reviews.toast.updateFailed"));
    }
  };

  const getProviderName = (r: any) => {
    const p = r.provider;
    if (!p) return t("reviews.defaultProvider");
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8">
        {[
          { label: t("reviews.stats.total"), value: reviews.length, color: "from-blue-500 to-cyan-600" },
          { label: t("reviews.stats.approved"), value: reviews.filter((r) => r.status === "approved").length, color: "from-green-500 to-emerald-600" },
          { label: t("reviews.stats.flagged"), value: reviews.filter((r) => r.status === "flagged").length, color: "from-orange-500 to-red-600" },
          { label: t("reviews.stats.hidden"), value: reviews.filter((r) => r.status === "hidden").length, color: "from-gray-500 to-gray-600" },
        ].map((stat, index) => (
      ...
      <div className="mb-6 space-y-4">
        <div className="relative">
          ...
          <input type="text" placeholder={t("reviews.searchPlaceholder")} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-12 pr-4 py-3 bg-white border border-gray-200 rounded-lg focus:ring-2 focus:ring-cyan-500 shadow-sm" />
        </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl p-10 text-center shadow"><p className="text-gray-500">{t("reviews.empty")}</p></div>
      ) : (
                  ...
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        <h3 className="font-bold text-gray-800">{review.user?.fullName || t("reviews.defaultUser")}</h3>
                    ...
                    <span className={`px-3 py-1 rounded-full text-xs font-semibold ${getStatusBadge(review.status)}`}>
                      {t(`reviews.statusLabel.${review.status}`, { defaultValue: review.status })}
                    </span>
                  </div>
                ...
                <div className="flex gap-2 items-start">
                  {review.status !== "approved" && (
                    <button onClick={() => handleStatus(review._id, "approved")} className="px-3 py-1 bg-green-100 text-green-700 rounded-lg hover:bg-green-200 text-sm font-medium flex items-center gap-1">
                      <Eye className="w-4 h-4" /> {t("reviews.actions.approve")}
                    </button>
                  )}
                  {review.status !== "flagged" && (
                    <button onClick={() => handleStatus(review._id, "flagged")} className="px-3 py-1 bg-orange-100 text-orange-700 rounded-lg hover:bg-orange-200 text-sm font-medium flex items-center gap-1">
                      <Flag className="w-4 h-4" /> {t("reviews.actions.flag")}
                    </button>
                  )}
                  {review.status !== "hidden" && (
                    <button onClick={() => handleStatus(review._id, "hidden")} className="px-3 py-1 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 text-sm font-medium flex items-center gap-1">
                      <EyeOff className="w-4 h-4" /> {t("reviews.actions.hide")}
                    </button>
                  )}
                </div>
              </div>
